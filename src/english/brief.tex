...
\begin{itemize}[leftmargin=0.15in, label={}]
    \small{\item{
    Data Engineer, HUINNO (Dec 2025 --). \textbf{I design and build the platform that makes research faster, and lay the MLOps pipeline on top of it piece by piece.} \\
    On bare-metal Kubernetes I operate every layer: network, storage, catalog, lineage, orchestration, distributed training, identity. \\
    I am attaching a reproducibility contract plus MLflow tracking and a model registry so research code can move onto KubeRay; the contract stamps version, content hash, code commit, environment lock and seed onto a dataset at run time. \\
    A quantified survey showed results surviving while the data, code and environment behind them did not — that is where this design started. \\
