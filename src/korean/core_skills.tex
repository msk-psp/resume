...
\begin{itemize}[leftmargin=0.15in, label={}]
    ...
    \\
    베어메탈 Kubernetes 위에 수집 → 레이크하우스 → 변환 → 계보 → 분산 학습 → 실험·모델 레지스트리까지 전 구간을 \textbf{Pulumi(Python IaC)}로 코드화해 혼자 구축하고 운영합니다. \textbf{10노드 · 상시 pod 235개 · 사내 서비스 23종}을 프로덕션으로 돌리고 있고 네트워크(Cilium) · 저장소(SeaweedFS) · 로그인(Dex OIDC → Kubernetes RBAC)까지 12개 스택을 하나도 빼놓지 않고 직접 짰습니다(스택 코드 변경의 93\%). 그 뒤를 잇는 분산 학습 · 실험 기록 · 모델 저장소는 \textbf{2026.08부터} 붙이고 있습니다. 문제가 코드 밖에 있을 때는 랜선을 갈거나 스위치를 바꾸고 남는 RAM · NVMe를 꽂는 일까지 \textbf{직접} 합니다.
    }
    \item{
    \textbf{데이터 파이프라인과 레이크하우스:}
    \\
    \textbf{Airflow 3.x + dbt(Cosmos) + Apache Iceberg} 메달리온 파이프라인을 \textbf{2026.03--07(5개월)}에 설계·구현하고 \textbf{DataHub}에 계보를 자동 기록합니다(ETL 저장소 커밋의 83\%, 공용 프레임워크의 93\% 직접 작성). 이전에는 AWS EMR/PySpark 위에 대규모 클라우드 파이프라인을 운영했습니다.
    }
    \item{
